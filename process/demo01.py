...
def remove_space(img, threshold = 40, kernel_size = 3, keepprop = 6, vborder = 3):
    # Filter horizontal
...
imgnms = glob.glob(f'{INPATH}/cck25k/*')

f = adf.id.sample(50).iloc[0]

imgls = [remove_border(cv2.imread(f'{INPATH}/cck25k/{f}_1.png')[:,:,::-1]) for t,f in adf.id.sample(500).iteritems()]
