...
validation_df = pd.read_csv(f"{INPATH}/written_name_validation_v2.csv")
train_df = pd.read_csv(f"{INPATH}/written_name_train_v2.csv")
preddf = pd.read_csv(f"{INPATH}/dread_pred.csv", names = ['FILENAME', 'ocr'])
validation_df.head()
# Display a few randomly chosen images along with their labels
PATH = f"{INPATH}/validation_v2/validation"

def process_image(fname):
    file_path = f'Datasets/Appen/validation_v2/validation/{fname}'
...
pool = Pool(8)
myls = validation_df.FILENAME.tolist()
...
validation_df['ocr'] = outls

if False:
    validation_df.to_feather(f"{INPATH}/written_name_validation_proc.feather")
validation_df = pd.read_feather(f"{INPATH}/written_name_validation_proc.feather")
validation_df['ocr'] = validation_df['ocr'].str.replace(r"\n", "")
validation_df['ocr'] = validation_df['ocr'].str.replace(r"\x0c", "")

validation_df['hammdist'] = validation_df.fillna('').apply(lambda x: edit_distance(x["IDENTITY"], x["ocr"]), axis=1)
validation_df = validation_df.query('hammdist < 2').reset_index(drop = True)
...
imgs = [cv2.imread(f"{PATH}/{fname}") for fname in validation_df.FILENAME.tolist()[250:1000]]
